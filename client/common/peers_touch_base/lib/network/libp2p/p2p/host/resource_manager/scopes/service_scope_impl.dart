...
import 'package:peers_touch_base/network/libp2p/core/peer/peer_id.dart'; // Added for concrete PeerId
import 'package:peers_touch_base/network/libp2p/p2p/host/resource_manager/limit.dart';
import 'package:peers_touch_base/network/libp2p/p2p/host/resource_manager/limiter.dart'; // Added for Limiter
...
class ServiceScopeImpl extends ResourceScopeImpl implements ServiceScope {
  ...
  ResourceScopeImpl getPeerSubScope(PeerId peerId, Limiter limiter, ResourceScopeImpl systemScope) {
    return _peerSubScopes.putIfAbsent(peerId, () {
      final peerServiceLimit = limiter.getServicePeerLimits(name, peerId);
      final scopeName = 'service:$name-peer:${peerId.toString()}';
