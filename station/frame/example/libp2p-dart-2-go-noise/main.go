...
func main() {
	...
	if port == "" {
		port = "5002" // Different port from plaintext server
	}
	...
	// Use Noise security (default in go-libp2p)
	h, err := libp2p.New(
		libp2p.ListenAddrs(listen),
		libp2p.Security(noise.ID, noise.New), // Enable Noise security
	...
	if target != "" {
		...
		defer s.Close()
		if _, err := s.Write([]byte("Hello from go-libp2p with Noise!")); err != nil {
