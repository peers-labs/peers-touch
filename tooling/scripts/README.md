...
```bash
./tooling/scripts/dev-station.sh
```

### 2. 启动两个桌面客户端 (调试多客户端交互)

```bash
./tooling/scripts/dev-two-desktops.sh
```

### 3. 启动桌面 + 移动端

```bash
./tooling/scripts/dev-desktop-mobile.sh
```
...
./tooling/scripts/dev-full-stack.sh

# 双 Desktop 模式
./tooling/scripts/dev-full-stack.sh --two-desktops

# 包含 Mobile
./tooling/scripts/dev-full-stack.sh --with-mobile

# 只启动 Station
./tooling/scripts/dev-full-stack.sh --station-only
```
