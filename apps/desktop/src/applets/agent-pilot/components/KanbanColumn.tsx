...
export function KanbanColumn({ status, issues, tagsByIssue = {}, workspacesByIssue = {}, selectedIds = new Set(), onToggleSelect, onAddIssue, onIssueClick }: Props) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: 40 }}>
        {issues.map((issue) => (
          <IssueCard
            ...
            workspaceCount={(workspacesByIssue[issue.id] ?? []).length}
