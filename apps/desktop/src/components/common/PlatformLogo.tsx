...
import { Gitlab, Link2 } from 'lucide-react';
...
function LarkLogo({ size = 20 }: { size?: number }) {
  return (
...
export function PlatformLogo({ providerId, size = 20, color }: Props) {
  const id = providerId.toLowerCase();
  if (id === 'github') return <Github.Avatar size={size} />;
  if (id === 'google') return <Google.Avatar size={size} />;
  if (id === 'bytedance') return <ByteDance.Avatar size={size} />;
  if (id === 'lark' || id === 'lark_simulate') return <LarkLogo size={size} />;
  if (id === 'gitlab') return <Gitlab size={size} color={color} />;
