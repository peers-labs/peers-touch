...
|-----------|---------|----------|
| **`src/widgets/`** | **Atomic Components** | `Button`, `Card`, `TextBox`, `Avatar`. **Avatar**: pass only `actorId` (uid) and `fallbackName`; do not pass `avatarUrl`—resolution is done inside the component or via registered `AvatarResolver`. |
